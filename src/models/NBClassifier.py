...
class NBClassifier(base_models):
    ...
    def fit(self, X, y):
        numSamples, numFeatures = X.shape #assigment to numSamples and numFeature the value of shape
        self._classes = np.unique(y) #obtain the unique value for each classes
        numClasses = len(self._classes) #assigment the length to classes

        #np.float64 same float but an accurate representation of the values
        #np.zeros creates an array of (n,m) size, and typenp.float64 and fills it with ceros
        ...
        for i in self._classes:
            ...
            self._priors[i] = X_i.shape[0] / float(numSamples)
